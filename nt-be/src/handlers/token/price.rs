...
async fn fetch_near_price(http_client: &reqwest::Client) -> Result<(f64, String), String> {
        ...
        .await
    {
        if let Ok(data) = response.json::<CoinGeckoResponse>().await {
            if let Some(price) = data.near.and_then(|n| n.usd) {
    ...
    if let Ok(response) = http_client
        ...
        .await
    {
        if let Ok(data) = response.json::<CryptoCompareResponse>().await {
    ...
    match http_client.get(&url).send().await {
        Ok(response) => {
            if let Ok(data) = response.json::<NearBlocksTokenResponse>().await {
                if let Some(contract) = data.contracts.first() {
                    if let Some(price_str) = &contract.price {
